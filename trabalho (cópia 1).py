...
class Map:
	...
	def plot_point(self, lat, lon, radius):
		...
		self.map.plot(x, y, 'orange', markersize=radius)
		plt.draw()
class AuthHandler:
	...
	def __init__(self, config_file="twitter.ini"):
		...
		self.api = tweepy.API(self.__auth, wait_on_rate_limit=True, wait_on_rate_limit_notify=True, retry_count=10, retry_delay=5, retry_errors=5)
		self.connected = True
